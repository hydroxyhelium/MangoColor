...
class Generator(nn.Module):
    def __init__(self, BUFFER_SIZE, BATCH_SIZE, IMG_WIDTH, IMG_HEIGHT):
        """
        ...
        self.downsample_layer1, _ = self.downsample(3, 32,4, apply_batchnorm=False) ## (batch_size, 32, 216, 216)
        self.downsample_layer2, _ = self.downsample(32, 64, 4) ## (batch_size, 64, 128, 128)
        self.downsample_layer3, _ = self.downsample(64, 128, 4) ## (batch_size, 128, 64, 64)
        self.downsample_layer4, _ = self.downsample(128, 256, 4) ## (batch_size, 256, 32, 32)
        self.downsample_layer5, _ = self.downsample(256, 512, 4) ## (batch_size, 512, 16, 16)
        self.downsample_layer6, _ = self.downsample(512, 512, 4) ## (batch_size, 512,8,8)
        self.downsample_layer7, _ = self.downsample(512, 512, 4) ## (batch_size, 512,4, 4)
        self.downsample_layer8, _ = self.downsample(512, 512, 4) ## (batch_size, 512,2,2)
        self.downsample_layer9, _ = self.downsample(512, 512, 4) ## (batch_size, 512, 1, 1)
        ...
        self.upsample_layer1,_ = self.upsample(512,512, 4, apply_dropout=True) ##(batch_size, 1024, 2, 2)
        self.upsample_layer2,_ = self.upsample(1024, 512, 4, apply_dropout=True) ##(batch_size, 1024, 4,4)
        self.upsample_layer3,_ = self.upsample(1024, 512, 4, apply_dropout=True) ##(batch_size, 1024,8,8)
        self.upsample_layer4,_ = self.upsample(1024, 512, 4) ##(batch_size, 1024,16,16)
        self.upsample_layer5,_ = self.upsample(1024, 512, 4) ##(batch_size, 1024, 32, 32)
        self.upsample_layer6,_ = self.upsample(1024, 256, 4) ##(batch_size, 512, 64,64)
        self.upsample_layer7,_ = self.upsample(512, 128, 4) ##(batch_size, 256, 128,128)
        self.upsample_layer8,_ = self.upsample(256, 64, 4) ##(batch_size, 128, 256,256)
        ...
        self.final_layer = torch.nn.ConvTranspose2d(128, 3, 4, stride=2, padding='same')
